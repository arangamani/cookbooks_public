...
2. prepare_db: This recipe prepares the database to be used for mysql based etherpadlite. It also installs the latest version of ruby from source as the mysql gem requires version 1.8.7 or higher. Once ruby is installed, the mysql gem is also installed. And then a small script is used to create a database named 'etherpadlite'.
3. start_pad: This recipe installs daemonize program and then starts etherpadlite using daemonize. The daemonize uses a log file to catch the output. This daemonize also stores the pid of the started process in a pid file which is used later while stopping the pad.
4. stop_pad: This recipe kills the pad with the help of the pid file created in the last step.
